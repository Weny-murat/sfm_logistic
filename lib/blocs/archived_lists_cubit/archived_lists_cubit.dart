...
class ArchivedListsCubit extends Cubit<List<LoadingList>> {
  final NetworkService network;
  ArchivedListsCubit({required this.network}) : super(<LoadingList>[]);
  ...
  Future<DataResult> getList(int id) async {
    return await network.get(
      '/api/LoadingList/$id',
      parser: (json) => LoadingList.fromJson(json),
    );
  }

  Future<DataResult> fetchLists() async {
    final result = await network.get(
      '/api/LoadingList/frozen',
      parser: (json) => LoadingListsResponse.fromMap(json),
    );
    switch (result) {
